...
export function makeProfile(repo, platform) {
  ...
  const base = path.resolve(process.env.COUNCIL_SMOKE_TMP || os.tmpdir());
  fs.mkdirSync(base, {recursive:true});
  const tmp = fs.mkdtempSync(path.join(base, 'council-smoke-'));
  ...
  for (const key of Object.keys(process.env)) if (key.startsWith('COUNCIL_') && key !== 'COUNCIL_SMOKE_TMP') delete process.env[key];
  ...
  const copyNode = dest => { fs.mkdirSync(path.dirname(dest), {recursive:true}); fs.copyFileSync(process.execPath, dest); return dest; };
  // Native fake executables are Node itself: --version is safe, any vendor argv fails.
  const binaries = {node:process.execPath, ...platform.systemBinaries(),
    claude:copyNode(path.join(app, platform.expectedImage('claude'))),
    codex_js:put(path.join(app, 'codex.js'), "if(process.argv.slice(2).join(' ') !== '--version') process.exit(97); console.log('codex fixture');\n")};
  if (platform.agyBinaryRoot()) binaries.agy = copyNode(path.join(platform.agyBinaryRoot(), platform.expectedImage('agy')));
  else binaries.agy = copyNode(path.join(app, platform.expectedImage('agy')));
  ...
  function walk(dir) { for (const e of fs.readdirSync(dir, {withFileTypes:true})) {
    ...
    if (e.isDirectory()) walk(p); else files[path.relative(path.join(repo,'src'),p).replaceAll('\\','/')] = crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');
  } }
