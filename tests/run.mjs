...
import '../tools/stage-release.test.mjs';
let failed = 0;
for (const suite of ['tier0/smoke.mjs', 'installer/unit/run.mjs', 'installer/run.mjs', 'trust/run.mjs']) {
...
console.log('gate: ' + (failed ? 'FAIL (' + failed + ' suites failed)' : 'PASS (4 suites)'));
