...
export default async function(test) {
  await test('native layout on disk', async root => {
    ...
    assert.equal(dirs.app, path.join(dirs.root,'app',version.APP_VERSION));
    assert.equal(appdirs({ env:{ ...process.env, HOME:root, USERPROFILE:root, LOCALAPPDATA:root, XDG_STATE_HOME:root, COUNCIL_PLATFORM:'' }, appVersion:'9.9.9' }).app, path.join(dirs.root,'app','9.9.9'));
  });
  await test('darwin override uses POSIX separators', async root => {
    ...
    assert.ok(!d.config.includes('\\')); assert.ok(!d.app.includes('\\'));
  });
